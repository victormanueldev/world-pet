...
class TestRegistrationLoginFlow:
    ...
    async def test_full_registration_login_access_flow(self) -> None:
        ...
        with (
            patch("app.api.v1.endpoints.auth.auth_service") as mock_auth_service,
            patch("app.api.v1.endpoints.auth.get_db") as mock_get_db,
            patch("app.dependencies.auth.get_db") as mock_deps_get_db,
        ):
            mock_db = AsyncMock()
            mock_get_db.return_value = mock_db
            mock_deps_get_db.return_value = mock_db
...
class TestRoleBasedAccessControl:
    ...
    async def test_admin_can_create_tenant(self) -> None:
        ...
        with (
            patch("app.api.v1.endpoints.tenants.tenant_service") as mock_service,
            patch("app.dependencies.auth.get_db") as mock_deps_db,
        ):
            mock_db = AsyncMock()
    ...
    async def test_non_admin_cannot_create_tenant(self) -> None:
        ...
        with patch("app.dependencies.auth.get_db") as mock_deps_db:
            mock_db = AsyncMock()
            mock_deps_db.return_value = mock_db
