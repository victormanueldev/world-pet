...
@pytest.fixture
def mock_postgres_container():
    """Mock postgres container with common operations."""
    container = MagicMock()
...
@pytest.fixture
def mock_postgres(mock_postgres_container, mocker):
    """Patch get_container to return mock."""
    mocker.patch(
        "app.db.postgres.get_container",
        return_value=mock_postgres_container
    )
    return mock_postgres_container


@pytest.fixture
def mock_postgres_unavailable(mocker):
    """Simulate postgres being unavailable."""
    mocker.patch(
        "app.db.postgres.get_container",
        return_value=None
    )
...
def sample_project_doc():
    """Sample project document as stored in postgres."""
    return {
        "id": "proj-123",
        "name": "Test Project",
...
```python
@pytest.fixture
def mock_postgres_async(mock_postgres_container, mocker):
    """Mock the async wrapper functions."""
    async def mock_upsert(doc, partition_key):
        return mock_postgres_container.upsert_item(doc)
    
    async def mock_get(doc_id, partition_key):
        return mock_postgres_container.read_item(item=doc_id, partition_key=partition_key)
    
    async def mock_delete(doc_id, partition_key):
        mock_postgres_container.delete_item(item=doc_id, partition_key=partition_key)
        return True
    
    async def mock_query(doc_type, partition_key=None, extra_filter=None, parameters=None):
        return list(mock_postgres_container.query_items())
    
    mocker.patch("app.db.postgres.upsert_document", side_effect=mock_upsert)
    mocker.patch("app.db.postgres.get_document", side_effect=mock_get)
    mocker.patch("app.db.postgres.delete_document", side_effect=mock_delete)
    mocker.patch("app.db.postgres.query_documents", side_effect=mock_query)
    mocker.patch("app.db.postgres.get_container", return_value=mock_postgres_container)
...
class TestProjectServiceCreate:
    ...
    async def test_create_persists_document_with_correct_structure(
        self, mock_postgres_async, project_create_data
    ):
        # Act
        result = await project_service.create(project_create_data, author_id="user-1")
        
        # Assert - verify upsert was called
        mock_postgres_async.upsert_item.assert_called_once()
        persisted_doc = mock_postgres_async.upsert_item.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_create_generates_uuid_id(self, mock_postgres_async, project_create_data):
        result = await project_service.create(project_create_data, author_id="user-1")
    ...
    @pytest.mark.asyncio
    async def test_create_sets_created_at_to_now(self, mock_postgres_async, project_create_data):
...
class TestProjectServiceGetById:
    ...
    async def test_get_by_id_returns_project_when_found(
        self, mock_postgres_async, sample_project_doc
    ):
        mock_postgres_async.read_item.return_value = sample_project_doc
    ...
    @pytest.mark.asyncio
    async def test_get_by_id_returns_none_when_not_found(self, mock_postgres_async):
        from azure.cosmos.exceptions import CosmosResourceNotFoundError
        mock_postgres_async.read_item.side_effect = CosmosResourceNotFoundError(
    ...
    @pytest.mark.asyncio
    async def test_get_by_id_returns_none_when_postgres_unavailable(
        self, mock_postgres_unavailable
    ):
        result = await project_service.get_by_id("proj-123", workspace_id="ws-456")
...
class TestProjectServiceUpdate:
    ...
    @pytest.mark.asyncio
    async def test_update_sets_updated_at(self, mock_postgres_async, sample_project_doc):
        mock_postgres_async.read_item.return_value = sample_project_doc
...
class TestProjectServiceDelete:
    ...
    @pytest.mark.asyncio
    async def test_delete_returns_true_on_success(self, mock_postgres_async):
        result = await project_service.delete("proj-123", workspace_id="ws-456")
        
        assert result is True
        mock_postgres_async.delete_item.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_delete_returns_false_when_not_found(self, mock_postgres_async):
        from azure.cosmos.exceptions import CosmosResourceNotFoundError
        mock_postgres_async.delete_item.side_effect = CosmosResourceNotFoundError(
            status_code=404, message="Not found"
        )
...
class TestProjectServiceList:
    ...
    async def test_list_returns_all_projects_in_workspace(
        ...
        mock_postgres_async.query_items.return_value = iter([
            sample_project_doc,
    ...
    @pytest.mark.asyncio
    async def test_list_returns_empty_when_no_projects(self, mock_postgres_async):
        mock_postgres_async.query_items.return_value = iter([])
...
For tests against real Postgres (emulator or dev instance):
...
pytestmark = pytest.mark.skipif(
    os.getenv("POSTGRES_ENDPOINT") is None,
    reason="POSTGRES_ENDPOINT not configured"
)
...
def postgres_container():
    ...
    from app.db.postgres import get_container, reset_connection
    reset_connection()
...
@pytest.fixture
def cleanup_test_docs(postgres_container):
    """Clean up test documents after each test."""
    created_ids = []
    yield created_ids
    for doc_id, partition_key in created_ids:
        try:
            postgres_container.delete_item(doc_id, partition_key)
        except Exception:
            pass
